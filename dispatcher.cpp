...
Dispatcher::Dispatcher(std::vector<Robot *> &robotList, std::vector<Workbench *> &workbenchList, std::unordered_map<int, std::vector<Workbench *> *> &workbenchTypeMap, std::unordered_map<int, std::vector<Task *> *> &workbenchIdTaskMap, int **accessMap) : robotList(robotList), workbenchList(workbenchList), workbenchTypeMap(workbenchTypeMap), workbenchIdTaskMap(workbenchIdTaskMap), taskTypeMap{}, taskChainQueueMap{}
{
    ...
    this->tempQueue = new std::priority_queue<TaskChain *, std::vector<TaskChain *>, CompareChain>();
    this->accessMap = accessMap;
    for (auto rb : robotList)
    {
        taskChainQueueMap[rb] = new std::priority_queue<TaskChain *, std::vector<TaskChain *>, CompareChain>();
    }
...
void Dispatcher::clearChainMap(Robot *rb)
{
    ...
    {
        std::priority_queue<TaskChain *, std::vector<TaskChain *>, CompareChain> *queue = (*taskChainQueueMap.find(rb)).second;
        while (!queue->empty())
        {
 ...
 *
 */
void Dispatcher::copyQueue(std::priority_queue<TaskChain *, std::vector<TaskChain *>, CompareChain> *source, std::priority_queue<TaskChain *, std::vector<TaskChain *>, CompareChain> *target)
{
    while (!target->empty())
    {
        target->pop();
    ...
    if (!freeRobots.empty())
    {
        ...
        while (!isQueueMapEmpty(nullptr))
        {
            ...
            for (Robot *rb : robotList)
            {
                ...
                std::priority_queue<TaskChain *, std::vector<TaskChain *>, CompareChain> *queue = taskChainQueueMap[rb];
